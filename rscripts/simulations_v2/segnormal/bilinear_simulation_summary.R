...
out.rs <- "autocal_simulation_segnormal//"
    ...
    for (i in seq(1, reps))
    {
      ...
      # --------------------
                               ...
                               episd, "_", i, ".Rdata")))
        {
          ...
          model4.ests.df <- data.frame(t(model4.ests[c("N0_ad", "Z1", "R1", "alpha", "beta", "bp", "log_sig_ga", "Z1_t", "log_c")]))
...
g <- ggplot(aes(y = Est, x = Age, col = factor(Iter)), data = model4.Gs.all) +
...
out.name <- paste0(bs.out, "/curve_ests")
png(filename = paste0(out.name, ".png"), 
    bg = "white", width =  18, height = 14, units = 'in', res = 300)
print(g)
dev.off()
