...
png(filename = paste0(out.name, ".png"), 
    bg = "white", width =  14, height = 14, units = 'in', res = 300)
plotAbund(N.est.all, sds = sds, fracs = fracs, max.iter = 50, maxy = 2300)
dev.off()
...
png(filename = paste0(out.name, ".png"), 
    bg = "white", width =  14, height = 14, units = 'in', res = 300)
plotAbund(model1.ests.all, MDL = 1, sds = sds, fracs = fracs, max.iter = 50,
          maxy = 2300)
dev.off()
...
png(filename = paste0(out.name, ".png"), 
    bg = "white", width =  14, height = 14, units = 'in', res = 300)
plotAbund(model4.Ns.all, sds = sds, fracs = fracs, max.iter = 50,
          maxy = 2300)
dev.off()
...
Avgs <- data.frame(N.est.all %>% group_by(SmpFrc, EpiSd) %>% dplyr::summarise(MeanVarRat = mean(Va_VG_A),
                                                                              ...
                                                                              MeanHSP   = mean(HSP)))
scn.avgs <- c("0.0025, 2, Nsamp = 300, NHSP = 70, V(A):V(G|A) ~ 4:1",
              "0.0025, 4, Nsamp = 300, NHSP = 70, V(A):V(G|A) ~ 1:1",
              "0.0025, 8, Nsamp = 300, NHSP = 70, V(A):V(G|A) ~ 1:4",
              "0.0035, 2, Nsamp = 420, NHSP = 140, V(A):V(G|A) ~ 4:1",
              "0.0035, 4, Nsamp = 420, NHSP = 140, V(A):V(G|A) ~ 1:1",
              "0.0035, 8, Nsamp = 420, NHSP = 140, V(A):V(G|A) ~ 1:4",
              "0.0045, 2, Nsamp = 530, NHSP = 220, V(A):V(G|A) ~ 4:1",
              "0.0045, 4, Nsamp = 530, NHSP = 220, V(A):V(G|A) ~ 1:1",
              "0.0045, 8, Nsamp = 530, NHSP = 220, V(A):V(G|A) ~ 1:4")
...
data.frame(all.ests.bta %>% group_by(SmpFrc, EpiSd) %>% dplyr::summarise(MN = mean(Beta),
                                                                         Md = median(Beta),
                                                                              SD = sd(Beta)))

all.ests.bta %>% group_by(SmpFrc, EpiSd) %>% dplyr::summarise(mn = mean(Beta),
                                                              med = median(Beta),
                                                              sd = sd(Beta))
...
out.name <- paste0(bs.out, "/beta_estimates")
png(filename = paste0(out.name, ".png"), 
...
all.ests.bta %>% group_by(SmpFrc, EpiSd) %>% dplyr::summarise(mn = mean(alpha),
                                                              med = median(alpha),
                                                              sd = sd(alpha))
