...
#dyn.unload(dynlib("code/cpp/epi_auto_deconv_nllk_lcky_known_age_v2"))
compile("code/cpp/epi_auto_deconv_nllk_lcky_known_age_v2.cpp")
dyn.load(dynlib("code/cpp/epi_auto_deconv_nllk_lcky_known_age_v2"))

#dyn.load(dynlib("code/cpp/epi_auto_deconv_nllk_lcky_known_age"))
...
fitlcky.l = nlminb(epi_autocal_nllk_lcky_l$par,
...
save(repo.lcky,  file = paste0(out.fl, "/model_5_rep", i, ".Rdata"))
...
plot(b_range, exp(abund) , ylim = c(0,10000), typ = 'line', lwd = 2)
...
mvn.draws.sub <- mvn.draws[, c("alpha", "Beta")]
mx.age <- 30
...
for (a in seq(0, mx.age))
{
  ...
  {
    mu <- mvn.draws.sub[n,1] + mvn.draws.sub[n, 2] * a
    
    res[n, a+1] <- rnorm(1, mu, episd)
  }
}
