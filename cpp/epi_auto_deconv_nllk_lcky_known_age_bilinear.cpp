...
Type objective_function<Type>::operator() ()
{
  ...
  // DATA_SCALAR(alpha);                            // Intercept for the linear model
  // DATA_SCALAR(sig_ga);                           // Variance of g-score given age
  DATA_SCALAR(fng_rate);                            // Adjustment for false negative rate
  ...
  PARAMETER(log_sig_ga);         // Variance of g-score given age. Not logged as we fix it!
  ...
  std::cout << "Years total " << ghat << std::endl;
  ...
  */
  ...
  for (byr_1 = 0; byr_1 < yrs_tot; byr_1++)
  {
    for (byr_2 = 0; byr_2 < yrs_tot; byr_2++)
    {
      ...
      {
        Type cumul_surv = exp(-Z1_t * (byr_2 - byr_1)); 
        hsp_prob(byr_1, byr_2) = c_t * 4 * fng_rate * cumul_surv / N_ad[byr_2];
      }
    }
  }
